...
#  Updated: 2026/02/03 09:26:43 by rruiz           ###   ########.fr        #
#                                                                           #
# ************************************************************************* #
    ...
    while True:
        ...
        while True:
            ...
            if "N" in directions and generator.maze[(x, y - 2)] == MAZE.wall:
                if (x, y - 2) not in generator.fourtytwo_coord and (x, y - 1) not in generator.fourtytwo_coord:
                    neighbors["N"] = (x, y - 2)
            if "E" in directions and generator.maze[(x + 2, y)] == MAZE.wall:
                if (x + 2, y) not in generator.fourtytwo_coord and (x + 1, y) not in generator.fourtytwo_coord:
                    neighbors["E"] = (x + 2, y)
        ...
        for ty in range(1, generator.height - 1, 2):
            for tx in range(1, generator.width - 1, 2):
                if generator.maze[(tx, ty)] == MAZE.wall and (tx, ty) not in generator.fourtytwo_coord:
                    potential_neighbors = []
                    if generator.maze.get((tx, ty - 2)) != MAZE.wall and generator.maze.get((tx, ty - 2)) is not None and (tx, ty - 1) not in generator.fourtytwo_coord:
                        potential_neighbors.append((tx, ty - 1))
                    if generator.maze.get((tx + 2, ty)) != MAZE.wall and generator.maze.get((tx + 2, ty)) is not None and (tx + 1, ty) not in generator.fourtytwo_coord:
